...
impl GlobalSettingsQueries {
    ...
    pub async fn get_user_settings(pool: &PgPool, user_id: i32) -> Result<Option<DbGlobalSettings>, sqlx::Error> {
        // First try to get settings by user_id (user-specific settings)
        ...
        if let Some(row) = user_row {
            return Ok(Some(DbGlobalSettings {
                id: row.try_get("id")?,
                user_id: row.try_get("user_id")?,
                account_id: row.try_get("account_id")?,
                company_name: row.try_get("company_name")?,
        ...
        // If no user-specific settings, try to get account settings as fallback
        let user = UserQueries::get_user_by_id(pool, user_id as i64).await?;
        if let Some(user) = user {
            if let Some(acc_id) = user.account_id {
    ...
    pub async fn update_user_settings(pool: &PgPool, user_id: i32, settings: UpdateGlobalSettings) -> Result<DbGlobalSettings, sqlx::Error> {
        ...
        let row = if account_id.is_some() {
            // Account users: try UPDATE first, then INSERT
            let update_result = sqlx::query(
                ...
                UPDATE global_settings SET
                    ...
                    logo_url = $6,
                    force_2fa_with_authenticator_app = COALESCE($7, force_2fa_with_authenticator_app),
                    add_signature_id_to_the_documents = COALESCE($8, add_signature_id_to_the_documents),
                    require_signing_reason = COALESCE($9, require_signing_reason),
                    allow_typed_text_signatures = COALESCE($10, allow_typed_text_signatures),
                    allow_to_resubmit_completed_forms = COALESCE($11, allow_to_resubmit_completed_forms),
                    allow_to_decline_documents = COALESCE($12, allow_to_decline_documents),
                    remember_and_pre_fill_signatures = COALESCE($13, remember_and_pre_fill_signatures),
                    require_authentication_for_file_download_links = COALESCE($14, require_authentication_for_file_download_links),
                    combine_completed_documents_and_audit_log = COALESCE($15, combine_completed_documents_and_audit_log),
                    expirable_file_download_links = COALESCE($16, expirable_file_download_links),
                    enable_confetti = COALESCE($17, enable_confetti),
                    completion_title = COALESCE($18, completion_title),
                    completion_body = COALESCE($19, completion_body),
                    redirect_title = COALESCE($20, redirect_title),
                    redirect_url = COALESCE($21, redirect_url),
                    updated_at = $22
                WHERE account_id = $2
            ...
            if let Some(row) = update_result {
                ...
                // For users without account_id: try UPDATE first, then INSERT if not found
                let update_result = sqlx::query(
                    r#"
                    UPDATE global_settings SET
                        ...
                        logo_url = $5,
                        force_2fa_with_authenticator_app = COALESCE($6, force_2fa_with_authenticator_app),
                        add_signature_id_to_the_documents = COALESCE($7, add_signature_id_to_the_documents),
                        require_signing_reason = COALESCE($8, require_signing_reason),
                        allow_typed_text_signatures = COALESCE($9, allow_typed_text_signatures),
                        allow_to_resubmit_completed_forms = COALESCE($10, allow_to_resubmit_completed_forms),
                        allow_to_decline_documents = COALESCE($11, allow_to_decline_documents),
                        remember_and_pre_fill_signatures = COALESCE($12, remember_and_pre_fill_signatures),
                        require_authentication_for_file_download_links = COALESCE($13, require_authentication_for_file_download_links),
                        combine_completed_documents_and_audit_log = COALESCE($14, combine_completed_documents_and_audit_log),
                        expirable_file_download_links = COALESCE($15, expirable_file_download_links),
                        enable_confetti = COALESCE($16, enable_confetti),
                        completion_title = COALESCE($17, completion_title),
                        completion_body = COALESCE($18, completion_body),
                        redirect_title = COALESCE($19, redirect_title),
                        redirect_url = COALESCE($20, redirect_url),
                        updated_at = $21
